...
class WordificationHubScreen extends StatelessWidget {
  ...
  static const String pageAdKey = 'inspire.wordification';
  ...
  Widget build(BuildContext context) {
    ...
    final wordificationInsertSections =
        _extractWordificationDynamicSections(inspire);
                  ...
                  children: [
                    _SectionHeader(
                      title: _resolveScreenText(
                        inspire,
                        const [
                          'wordification_featured_title',
                          'featured_title'
                        ],
                        fallback: 'Featured Wordification',
                      ),
                      subtitle: _resolveScreenText(
                        inspire,
                        const [
                          'wordification_featured_subtitle',
                          'featured_subtitle'
                        ],
                        fallback:
                    ...
                    _SectionHeader(
                      title: _resolveScreenText(
                        inspire,
                        const ['wordification_all_title', 'all_title'],
                        fallback: 'All Wordification',
                      ),
                      subtitle: _resolveScreenText(
                        inspire,
                        const ['wordification_all_subtitle', 'all_subtitle'],
                        fallback: 'Browse all available posts below.',
                      ),
                    ),
                    const SizedBox(height: 12),
                    if (items.isEmpty && wordificationInsertSections.isEmpty)
                      const _EmptyCard(
                      ...
                      _WordificationArticleFlow(
                        entries: listEntries,
                        insertSections: wordificationInsertSections,
  ...
  static Widget _buildArticleCard(
    ...
    return _ArticleListCard(
      ...
      pillText: _resolvePillText(m, fallback: 'Wordification'.toUpperCase()),
      icon: Icons.lightbulb_rounded,
      onTap: () async {
        ...
        context.push(
          '/articles/detail',
          extra: {'id': id, 'source': 'wordification'},
        );
      },
    );
  }
  ...
  static List<Map<String, dynamic>> _extractItems(
    ...
    for (final key in const ['wordification']) {
  ...
  static List<HubDynamicSection> _extractWordificationDynamicSections(
    Map<String, dynamic> inspire,
  ) {
    return DynamicInsertBlockResolver.extractSections(
      inspire,
      targetBuckets: const ['wordification', 'inspire_wordification'],
    );
  }

  static bool _isBackendControlledWordificationInsert(
    Map<String, dynamic> section,
  ) {
    final id = section['id'];
    final isRealDestinationSection = id is int && id > 0;
    if (!isRealDestinationSection) return false;
    ...
    String norm(String raw) => _normalizeBuilderKey(raw);
    ...
    final title =
        norm((section['title'] ?? settings['title'] ?? '').toString());
    final placement = norm(value('placement_type').isNotEmpty
        ? value('placement_type')
        : value('placement'));
    final targetPage = norm(value('target_page'));
    final insertTarget = norm(value('insert_target_bucket').isNotEmpty
        ? value('insert_target_bucket')
        : value('insert_target'));
    final parent = norm(value('parent_section_key').isNotEmpty
        ? value('parent_section_key')
        : value('parent_bucket').isNotEmpty
            ? value('parent_bucket')
            : value('parent'));
    ...
    final isRootWordification = key == 'wordification' ||
        key == 'inspire_wordification' ||
        title == 'wordification';
    if (isRootWordification) return false;

    final targetsWordification = insertTarget == 'wordification' ||
        parent == 'wordification' ||
        targetPage == 'wordification' ||
        placement == 'wordification';

    final isInsertPlacement = placement == 'inside_content_list' ||
        placement == 'after_featured' ||
        placement == 'before_list' ||
        placement == 'after_item' ||
        placement == 'end_of_page';

    final isRenderableInsert = engine.contains('short_video') ||
        sourceType.contains('short_video') ||
        layout.contains('short_video') ||
        engine.contains('quote') ||
        sourceType.contains('quote') ||
        layout.contains('quote') ||
        engine.contains('scripture') ||
        sourceType.contains('scripture') ||
        layout.contains('scripture') ||
        bucket == 'daily_scriptures' ||
        bucket == 'daily_quotes' ||
        bucket == 'short_videos' ||
        key.contains('bible_short') ||
        key.contains('daily_scripture');

    return targetsWordification && isInsertPlacement && isRenderableInsert;
  }
  ...
  static String _normalizeBuilderKey(String value) {
    return value
        .trim()
        .toLowerCase()
  ...
  static List<Map<String, dynamic>> _extractFeaturedItems(
    ...
    for (final key in const [
      'wordification_featured',
      'featured_wordification',
      'featured',
    ]) {
  ...
  static String _resolveScreenText(
    ...
    final meta = inspire['wordification_meta'];
    if (meta is Map) {
      for (final key in keys) {
          ...
          NotificationListener<ScrollNotification>(
            ...
            child: PageView.builder(
              ...
              itemBuilder: (context, i) {
                ...
                final title = WordificationHubScreen._firstNonEmpty([
                  (m['title'] ?? '').toString(),
                  (m['topic'] ?? '').toString(),
                  'Wordification',
                ]);
                final subtitle = WordificationHubScreen._firstNonEmpty([
                  (m['subtitle'] ?? '').toString(),
                  WordificationHubScreen._resolveMetaLine(m),
                ]);
                final excerpt = WordificationHubScreen._resolveExcerpt(m);
                ...
                return Padding(
                  ...
                  child: _FeaturedArticleCard(
                    ...
                    subtitle: subtitle,
                    excerpt: excerpt,
                    imageUrl: WordificationHubScreen._pickImage(m),
                    fallbackAsset: WordificationHubScreen._fallbackAssetFor(i),
                    icon: widget.icon,
                    pillText: WordificationHubScreen._resolvePillText(
                      m,
                    ...
                    onTap: () async {
                      await AdsService.instance.maybeShowInterstitialOnSafeNav(
                        context,
                        tabKey: WordificationHubScreen.pageAdKey,
                      );
                      if (!context.mounted) return;
                      context.push(
                        '/articles/detail',
...
class _WordificationArticleFlow extends StatelessWidget {
  final List<dynamic> entries;
  final List<HubDynamicSection> insertSections;
  final Widget Function(Map<String, dynamic> item, int itemIndex)
      buildArticleCard;

  const _WordificationArticleFlow({
    required this.entries,
    required this.insertSections,
  ...
  Widget build(BuildContext context) {
    ...
    final pendingSections = List<HubDynamicSection>.from(insertSections);

    var contentCount = 0;

    void addSectionsForCurrentPosition() {
      if (pendingSections.isEmpty) return;

      final ready = pendingSections
          .where(
            (section) =>
                contentCount >=
                WordificationHubScreen._sectionInsertAfter(section),
          )
          .toList(growable: false);

      for (final section in ready) {
        pendingSections.remove(section);
    ...
    for (final section in pendingSections) {
      if (children.isNotEmpty) children.add(const SizedBox(height: 14));
      children.add(
        DynamicHubSectionRenderer(
          section: section,
